  ...
  useEffect(() => {
    if (type == 1) {
      ...
      api.get(`connections`).then(({ data }) => {
        setSearchareas([{ name: "Selecionar linha" }, ...data]);
      });
    }
  }, [type]);
  ...
  return (
    ...
    >
      <div className="modal-header">
        <h5 className="modal-title" id="exampleModalLabel">{`Adicionar ${type == 1 ? 'Área de conhecimento' : 'Linha de pesquisa'}`}</h5>
      </div>
      ...
      <Form>
        <div className="modal-body" ref={reference}>
          ...
          <div className="input-block">
            <label>
              {`${type == 1 ? 'Área de conhecimento' : 'Linha de pesquisa'}`}
            </label>
            <select onChange={(e) => {
